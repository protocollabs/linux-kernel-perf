...
def _print_header():
    fmt = _fmt_header()
    print(fmt.format('SWITCHED-OUT', 'SWITCHED-IN', 'CPU', 'PID', 'TID', 'COMM', 'RUNTIME [us]', 'GAP LAST TID[us]'))
...
def _print_task_finish(task):
    ...
    print(fmt.format(task.time_out(), task.time_in(), task.cpu,
          c_pid_color, task.pid, c_pid_reset,
          c_tid_color, task.tid, c_tid_reset,
          c_comm_color, comm, c_comm_reset,
          task.runtime(unit='us'), last_gap))
